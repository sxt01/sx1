        ...
        for index, download in enumerate(list(download_dict.values())[START:], start=1):
            ...
            if download.status() not in [
                ...
                if download.status() == MirrorStatus.STATUS_CLONING:
                    ...
                    msg += f"\n {get_readable_file_size(download.processed_bytes())} of {download.size()}"
                msg += f" at {download.speed()} , ETA: {download.eta()} "
                try:
                    msg += f" | <b>S:</b> {download.aria_download().num_seeders}" \
                           f" | <b>P:</b> {download.aria_download().connections}"
                except:
                    pass
                try:
                    msg += f" | <b>S:</b> {download.torrent_info().num_seeds}" \
                           f" | <b>L:</b> {download.torrent_info().num_leechs}"
                except:
                    pass
                ...
                msg += f" | <b>Ratio: </b>{round(download.torrent_info().ratio, 3)}"
                msg += f" | {get_readable_time(download.torrent_info().seeding_time)}"
                msg += f"\n<code>/{BotCommands.CancelMirror} {download.gid()}</code>"
            else:
                msg += f"\n<b>Size: </b>{download.size()}"
            msg += "\n\n"
